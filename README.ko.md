...
> **diff가 무엇이 바뀌었는지 보여준다면, Where am I는 그 변경이 어디까지 이어지는지 보여줍니다.**
...
Where am I는 Git 호스팅, IDE, observability 도구를 대체하려는 제품이 아닙니다.
코드를 수정한 뒤 리뷰하거나 배포하기 전에 생기는 빈틈, 즉 **이 변경이 다른
어디에 영향을 주는가?**에 답합니다.

| 기존 방식 | 잘 보여주는 것 | Where am I가 더하는 것 |
| --- | --- | --- |
| GitHub 또는 PR diff | 한 리뷰 안에서 바뀐 라인 | PR을 만들기 전부터 확인하는 파일·저장소 간 영향 경로 |
| IDE 참조 찾기 | 지원 언어나 workspace 내부의 symbol 참조 | HTTP route, wrapper, handler, 문서, schema, 테스트를 넘나드는 연결 |
| 아키텍처 다이어그램 | 문서에 기록된 시점의 시스템 구조 | 오늘의 작업 트리나 폴더 기준점을 다시 스캔한 현재 코드 상태 |
| 호스팅형 코드 인텔리전스 | 조직 전체의 중앙 집중식 index | 계정·GitHub 연결·소스 업로드가 필요 없는 로컬 우선 실행 |
| 수동 리뷰 체크리스트 | 팀원이 기억하는 검증 지식 | 코드에서 반복 생성되는 클릭 가능한 근거 라인과 계약 검사 |

다음과 같은 상황에 유용합니다.

- 아직 커밋하지 않은 변경의 영향 범위를 확인할 때
- 프론트엔드, 백엔드, 문서와 테스트를 가로지르는 API 변경을 리뷰할 때
- 여러 Git 저장소나 일반 폴더를 하나의 시스템처럼 살펴볼 때
- 외부로 업로드하면 안 되는 비공개 소스에서 작업할 때
- 새 기여자에게 다음으로 살펴볼 지점을 근거와 함께 빠르게 안내할 때

그래프는 확인 가능한 정적 규칙과 소스 근거를 바탕으로 만들어집니다. 설명하기
어려운 점수나 생성형 요약 뒤에 숨기보다, 개발자의 판단을 돕는 것을 목표로 합니다.
